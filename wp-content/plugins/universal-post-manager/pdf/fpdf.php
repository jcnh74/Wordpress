...

{
	...
	if(!$a){
		...
		$a = getimagesize($file);
		if( !$a && strpos($file, 'http') !== FALSE ){
			$this->Error('[Error] '.$file.' Missing or Please turn on "allow_url_fopen" on your server to allow insert images in PDF Document, more info here: http://stackoverflow.com/questions/3694240/add-allow-url-fopen-to-my-php-ini-using-htaccess');
		}
		if(!$a){
			$a = getimagesize( './'. $file );
		}
	...
	if(!$a) $a = array ( 0 => 18, 1 => 13, 2 => 2, 3 => 'width="18" height="13"', 'bits' => 8, 'channels' => 3, 'mime' => 'image/jpeg' ) ;
