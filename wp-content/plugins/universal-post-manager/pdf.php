<?php 
error_reporting(0);
if( !get_option('pppm_save_pdf_img_show') ){
	$STR = preg_replace( '|<img\s+[^>]*>|i', '', $STR );
}
if( get_option('pppm_save_pdf_img_max_width') ){ $upm_max_image_width = get_option('pppm_save_pdf_img_max_width'); } else{ $upm_max_image_width = 500; }
//$pppm_encod = get_option('blog_charset');
//$STR = mb_convert_encoding($STR, "windows-1252","UTF-8");
//$STR = mb_convert_encoding($STR, "gb2312","UTF-8");
//$STR = mb_convert_encoding($STR, "KOI8-R","UTF-8");

if( get_option('pppm_save_pdf_rus') ){
...
$replace = array (  '^[li][$1]^','^[b][$1]^','^[b][$1]^','^[i][$1]^','^[u][$1]^','^[code][$1]^','^[_br_]^',
					'^[h1][$1]^','^[h2][$1]^','^[h3][$1]^','^[h4][$1]^');
################################################################################
...
foreach ( $img_array as $value ) {
	...
	if(strpos($src, '-150x150') !== FALSE ) {
		$new_src = str_replace('-150x150.', '.', $src);
		$local_src = upm_path( $new_src );
		if( file_exists( $local_src ) ){
			$src = $new_src;
		}
		list($x_width, $x_height) = getimagesize($local_src);
		$img_array_align[1] = '';
		$img_array_height[1] = $x_height;
		$img_array_width[1] = $x_width;
	}
	else{
	...
	////////////////////////////////////////////////////////////////////////////////
	if( (int)$upm_max_image_width < (int)$img_array_width[1] ){
		$img_array_height[1] = $img_array_height[1] * ( $upm_max_image_width / $img_array_width[1] );
		$img_array_width[1] = $upm_max_image_width;
	}
	////////////////////////////////////////////////////////////////////////////////
...
function upm_path( $file ){
	...
	if(!$f){
		...
		if( !$f && strpos($file, 'http') !== FALSE ){
			echo ('[UPM PDF Error] Please turn on "allow_url_fopen" on your server to allow insert images in PDF Document, more info here: http://stackoverflow.com/questions/3694240/add-allow-url-fopen-to-my-php-ini-using-htaccess');
		}
	}
	fclose($f);
	return $file;
...
function upm_alpha( $file ){

	$file = upm_path($file);
	$dir = basename(WP_CONTENT_URL) . '/uploads/upm/';
	$file_jpg = basename(WP_CONTENT_URL) . '/uploads/upm/' . str_replace('.png', '.jpg', strtolower(basename($file)));
...
function upm_clean( $file ){
	if( strpos( $file, basename(WP_CONTENT_URL) . '/uploads/upm/' ) !== FALSE ) {
		@unlink($file);
	}
}
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$header_text_1 = 'This page was exported from - ';
		...
		$this->Write(0, str_replace('&amp;','&',$header_text_2), $pppm_html_link );
		$this->SetFont('','',8);
		$this->SetTextColor(150,150,150);
	...
	function Footer()
	{
		...
		$footer_text_2 = ' Universal Post Manager ' ;
		$footer_text_3 = '] plugin from www.ProfProjects.com';
		$this->SetY(-10);
		//$this->Image(PPPM_PATH.'img/mini_icon.gif',($this->GetX()+1.5),($this->GetY()+1.4),4,3,'gif','http://profprojects.com');
		$this->SetFont('Times','I',8);
		$this->SetTextColor(150,150,150);
		$this->Write(6, $footer_text_1 );
		$this->SetTextColor(150,150,255);
		$this->Write(6, $footer_text_2, 'http://www.profprojects.com/?page=upm' );
		$this->SetTextColor(150,150,150);
		...
		switch( $row ){
				...
				if( $a_data[1] ) {
					...
					if( strpos( $a_data[2], 'img')>0 ) {
						...
						if( $a_data[3] ) {
							...
							if( strtolower(file_extension(trim($a_data[3]))) == 'png' ){ $file = upm_alpha( trim($a_data[3]) ); } else{ $file = trim($a_data[3]); }
			...
			case strpos( $row, 'img][')>0 : 
			{
				...
				if( $img_data[1] ) {
					...
					if( strtolower(file_extension(trim($img_data[1]))) == 'png' ){ $file = upm_alpha( trim($img_data[1]) ); } else{ $file = trim($img_data[1]); }
