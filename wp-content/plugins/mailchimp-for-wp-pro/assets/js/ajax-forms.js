		...
		success: function(response, status) {
			...
			if(response.success) {
				...
				if(e == 'error' && response.show_error && response.mailchimp_error) {
					$('<div class="mc4wp-alert mc4wp-notice" id="mc4wp-mailchimp-error"><strong>MailChimp returned this error:</strong><br>' + response.mailchimp_error + '<br><em>(this message is only visible to administrators</em></div>').insertAfter($message);
				}
			}
