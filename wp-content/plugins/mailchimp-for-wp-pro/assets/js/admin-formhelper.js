(function($) { 
	...
	*/
	(function() {
			...
			function toggleRow(rowId, enable) 
			{
				if(enable) {
					$("tr#mc4wp-" + rowId).removeClass('hidden').find(':input').removeAttr('disabled');
				} else {
					$("tr#mc4wp-" + rowId).addClass('hidden').find(':input').attr('disabled', 'disabled');
				}
			}
			...
			$inputDoubleOptin.change(function() {
				...
				toggleRow('send-welcome', (value != 1));
			});
			$inputUpdateExisting.change(function() {
				var value = ($(this).val() == '') ? $(this).data('inherited-value') : $(this).val();
				toggleRow('replace-interests', (value == 1));
			})
			...
			$lists.filter(':checked').each(function() {
				...
				for(var i = 0, fieldCount = fields.length; i < fieldCount; i++) {
					...
					if($mailchimpMergeFields.find("option[value='"+ field.tag +"']").length == 0) {
						...
						var text = (field.name.length > 25) ? field.name.substring(0, 25) + '..' : field.name;
						if(field.req) { text += '*'; }
				...
				for(var i = 0, groupingsCount = groupings.length; i < groupingsCount; i++) {
					...
					if($mailchimpGroupings.find("option[value='"+ grouping.id +"']").length == 0) {
						var text = (grouping.name.length > 25) ? grouping.name.substring(0, 25) + '..' : grouping.name;
